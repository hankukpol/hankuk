...
import { MorningSubjectOverview } from "./SubjectOverview";
...
export function MorningStudentReport({ report, mode, section, records }: { report: Report; mode: "admin" | "student"; section?: ReportSection; records?: ReactNode }) {
  ...
  const heading = <header>{!section && <h2 className="admin-section-title">{report.examType.name} 개인 분석</h2>}<p className="admin-help">{report.range.from} ~ {report.range.to}{mode === "admin" && report.student.name ? ` · ${report.student.name}` : ""}</p></header>;
  return <ReportDocumentProvider value={mode === "admin"}><div className="admin-flat-page admin-compact-workspace" data-report-root>
    ...
    {mode === "admin" && <div data-report-section="overview"><MorningSubjectOverview report={report} /></div>}
    ...
    {<section data-report-section="trend" className="admin-section"><h2 className="admin-section-title">과목별 추세</h2>
    ...
    <section data-report-section="subjects" className="admin-section"><h2 className="admin-section-title">과목별 복습 기록</h2>
    ...
    <section data-report-section="items" className="admin-section"><h2 className="admin-section-title">문항 분석</h2>
