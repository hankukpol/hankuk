...
export function PointRuleManager({ divisionSlug }: PointRuleManagerProps) {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const data = (await response.json()) as { rule?: PointRuleItem; error?: string };
  ...
  function renderRuleActions(rule: PointRuleItem) {
    return (
      <div className="flex items-center justify-end gap-2">
        ...
        <button type="button" onClick={() => void handleDelete(rule.id)} disabled={deletingId !== null} className="admin-icon-button text-admin-danger" aria-label={rule.name + " 삭제"} title="규칙 삭제">
