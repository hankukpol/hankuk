...
export interface SpecialLectureEditInitialData {
  id: string;
  name: string;
  lectureType: string;
...
export function SpecialLectureEditForm({ initial }: { initial: SpecialLectureEditInitialData }) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();
  const [formError, setFormError] = useState<string | null>(null);
  const [form, setForm] = useState<FormState>({
    name: initial.name,
    lectureType: (initial.lectureType as SpecialLectureType) ?? "SINGLE",
    examCategory: (initial.examCategory as ExamCategory | null) ?? "",
    startDate: isoToDateInput(initial.startDate),
    endDate: isoToDateInput(initial.endDate),
    isMultiSubject: initial.isMultiSubject,
    fullPackagePrice: initial.fullPackagePrice != null ? String(initial.fullPackagePrice) : "",
    hasSeatAssignment: initial.hasSeatAssignment,
    hasLive: initial.hasLive,
    hasOffline: initial.hasOffline,
    maxCapacityLive: initial.maxCapacityLive != null ? String(initial.maxCapacityLive) : "",
    maxCapacityOffline: initial.maxCapacityOffline != null ? String(initial.maxCapacityOffline) : "",
    waitlistAllowed: initial.waitlistAllowed,
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      try {
        const res = await fetch(`/api/special-lectures/${initial.id}`, {
          method: "PATCH",
        ...
        router.push(`/admin/settings/special-lectures/${initial.id}`);
        router.refresh();
      } catch (err) {
        setFormError(err instanceof Error ? err.message : "수정 실패");
      }
    });
  }
    ...
    <form onSubmit={handleSubmit} noValidate>
      <div className="rounded-[28px] border border-ink/10 bg-white shadow-sm">
        ...
        <div className="border-b border-ink/5 px-6 py-4">
          <h2 className="text-base font-semibold text-ink">특강 정보 수정</h2>
