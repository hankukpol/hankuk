  ...
  case p_app_key
    ...
    when 'study-hall' then
      ...
      if p_role_key = 'super_admin' then
        ...
        values (p_user_id::text, coalesce(p_full_name, coalesce(p_email, '운영자')), 'SUPER_ADMIN', null, true)
        ...
        insert into study_hall.admins (user_id, name, role, division_id, is_active)
        values (p_user_id::text, coalesce(p_full_name, coalesce(p_email, '운영자')), 'ASSISTANT', v_division_id, true)
        on conflict (user_id) do update
        set
          name = excluded.name,
          role = excluded.role,
        ...
        insert into study_hall.admins (user_id, name, role, division_id, is_active)
        values (p_user_id::text, coalesce(p_full_name, coalesce(p_email, '운영자')), 'ADMIN', v_division_id, true)
        on conflict (user_id) do update
        set
          name = excluded.name,
          role = excluded.role,
