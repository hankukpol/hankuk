...
function NewEventModal({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/exams/monthly", {
        method: "POST",
...
function StudentRegistrationModal({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/exams/monthly/${eventId}/registrations`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
...
export function MonthlyExamManager({ initialEvents }: { initialEvents: ExamEventRow[] }) {
  ...
  const fetchRegistrations = useCallback(async (eventId: string) => {
    ...
    try {
      const res = await fetch(
        `/api/exams/monthly/${eventId}/registrations?includeCancelled=true`,
      );
      const data = await res.json();
  ...
  async function togglePaid(reg: RegistrationRow) {
    ...
    const res = await fetch(
      `/api/exams/monthly/${selectedEventId}/registrations/${reg.id}`,
      {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  async function cancelRegistration(regId: string) {
    ...
    try {
      const res = await fetch(
        `/api/exams/monthly/${selectedEventId}/registrations/${regId}`,
        { method: "DELETE" },
      );
      if (res.ok) {
        setRegistrations((prev) =>
                  ...
                  {activeRegistrations.map((reg) => {
                    ...
                    return (
                      <tr key={reg.id} className="hover:bg-mist/50">
                        ...
                        <td className="px-4 py-3">
                          <Link
                            href={`/admin/exams/monthly/${selectedEventId}/receipt/${reg.id}`}
                            target="_blank"
