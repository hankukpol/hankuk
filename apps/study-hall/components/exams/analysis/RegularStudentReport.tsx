...
export type ReportSection = "diagnosis" | "trend" | "subjects" | "rank";

const value = (number: number | null | undefined, suffix = "") => number == null || !Number.isFinite(number) ? "집계 불가" : `${Number(number.toFixed(1))}${suffix}`;

export function RegularStudentReport({ report, mode, section }: { report: Report; mode: "admin" | "student"; section?: ReportSection }) {
  const show = (target: ReportSection) => !section || section === target;
  ...
  return <div className="admin-flat-page">
    ...
    {history && show("trend") && <section className="admin-section"><h2 className="admin-section-title">최근 6개월 개인 성적</h2>
    ...
    {show("subjects") && <section className="admin-section"><h2 className="admin-section-title">과목 균형과 학습 조언</h2><div className="admin-notice"><p>{stats.balance.assessment} (표준편차 {value(stats.balance.stdDev)})</p>{stats.advice.length ? <ul>{stats.advice.map((advice, index) => <li key={index}>{advice}</li>)}</ul> : <p>추가 학습 조언이 없습니다.</p>}</div></section>}
    {show("subjects") && <section className="admin-section"><h2 className="admin-section-title">외부 성적 분포</h2><DistributionBars distribution={report.distribution} /></section>}
    {show("subjects") && <div id="personal-items"><ItemAnalysisTable items={report.items} subjects={report.subjects} /></div>}
    {show("trend") && <section id="personal-trend" className="admin-section"><h2 className="admin-section-title">날짜별 추이</h2>{hasTrend ? <ExamScoreChart results={trend} variant={mode === "admin" ? "admin" : "portal"} /> : <p className="admin-empty-state">직전 시험이 없어 비교할 수 없습니다. 다음 시험부터 표시됩니다.</p>}</section>}
    {show("rank") && <section className="admin-section"><h2 className="admin-section-title">목표 대비</h2>{report.target ? <p className="admin-notice">목표 {value(report.target.targetScore)}점 · 목표와 차이 {value(report.target.gap)}점 ({value(report.target.gapPercent)}%)</p> : <p className="admin-help">등록된 목표 점수가 없습니다.</p>}{report.flags.length > 0 && <ul className="admin-help">{report.flags.map((flag, index) => <li key={`${flag.kind}-${index}`}>{flag.detail}</li>)}</ul>}</section>}
    {show("rank") && <section className="admin-section"><h2 className="admin-section-title">내 주변 석차</h2><p className="admin-help">반 내 앞 5명·뒤 4명을 익명으로 비교합니다. 수험번호 일부만 표시합니다.</p>
