...
const SUBJECT_RULES: Partial<Record<
  ExamType,
  ReadonlyArray<{
    name: string;
...
>> = {
  // 공채: 3과목 75문항 300점
  [ExamType.PUBLIC]: [
    { name: "소방학개론", questionCount: 25, pointPerQuestion: 4, maxScore: 100 },
    { name: "소방관계법규", questionCount: 25, pointPerQuestion: 4, maxScore: 100 },
    { name: "행정법총론", questionCount: 25, pointPerQuestion: 4, maxScore: 100 },
  ],
  // 구조 경채: 2과목 65문항 200점
  [ExamType.CAREER_RESCUE]: [
    { name: "소방학개론", questionCount: 25, pointPerQuestion: 4, maxScore: 100 },
    { name: "소방관계법규", questionCount: 40, pointPerQuestion: 2.5, maxScore: 100 },
  ],
  // 소방학과 경채: 구조와 동일 시험과목, 2과목 65문항 200점
  ...
  [ExamType.CAREER_EMT]: [
    { name: "소방학개론", questionCount: 25, pointPerQuestion: 4, maxScore: 100 },
    { name: "응급처치학개론", questionCount: 40, pointPerQuestion: 2.5, maxScore: 100 },
  ],
};
...
export interface ScoreResult {
  ...
  finalScore: number;
  hasCutoff: boolean;
  hasTotalCutoff: boolean; // 소방: 총점 60% 미만 과락
...
function sortAndValidateSubjects(examType: ExamType, subjects: SubjectMeta[]): SubjectMeta[] {
  ...
  const ordered = rule.map((required) => {
    ...
    if (
      ...
      throw new Error(`${required.name} 과목 설정이 소방 규정과 일치하지 않습니다.`);
    }
...
function scoreByAnswerMap(params: {
  ...
  if (hasTotalCutoff) {
    ...
    for (const score of scores) {
      score.isCutoff = true;
      score.bonusScore = 0;
      score.finalScore = score.rawScore;
    }
  }
  ...
  return {
    examType,
    totalScore,
    bonusScore: hasTotalCutoff ? 0 : totalBonusScore,
    finalScore: hasTotalCutoff ? totalScore : roundScore(totalScore + totalBonusScore),
    hasCutoff,
    hasTotalCutoff,
    scores,
    userAnswers,
...
export async function rescoreExam(examId: number, options?: RescoreOptions): Promise<RescoreResult> {
  ...
  const detailedMode =
    ...
    (options.examType === ExamType.PUBLIC || options.examType === ExamType.CAREER_RESCUE || options.examType === ExamType.CAREER_ACADEMIC || options.examType === ExamType.CAREER_EMT);
  ...
  for (const idChunk of submissionIdChunks) {
    ...
    for (const examType of requiredTypes) {
      const context = await loadScoringContext(prisma, examId, examType);
