  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-xl font-extrabold text-gray-900">강좌 관리</h2>
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <h3 className="text-sm font-bold text-gray-700">새 강좌 만들기</h3>
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-3">
            <input
              ...
              placeholder="강좌명"
              className="rounded-xl border border-slate-200 px-3 py-2.5 text-sm outline-none focus:border-slate-400 sm:col-span-2"
            />
            <select
          ...
          <div className="mt-3 flex flex-wrap gap-3">
            {FEATURE_LABELS.map((item) => (
              <label key={item.key} className="flex items-center gap-1.5 text-xs text-gray-600">
                <input
                  type="checkbox"
      ...
      <section className="overflow-hidden rounded-2xl bg-white shadow-sm">
        {loading ? (
          ...
          <p className="px-5 py-12 text-center text-sm text-gray-400">해당 강좌가 없습니다.</p>
        ) : (
                ...
                {filtered.map((course) => {
                  ...
                  return (
                    <tr key={course.id} className="hover:bg-slate-50/60">
                      ...
                      <td className="px-3 py-3.5 text-gray-500">{courseTypeLabel(course.course_type)}</td>
                      ...
                      <td className="hidden px-3 py-3.5 md:table-cell">
                        <div className="flex flex-wrap gap-1">
                          {tags.map((t) => (
                            <span key={t as string} className="rounded-md bg-slate-100 px-1.5 py-0.5 text-[10px] font-medium text-slate-500">
                              {t}
                            </span>
                          ))}
                        </div>
                      ...
                      <td className="px-5 py-3.5 text-right">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          <button
                            ...
                            className="rounded-lg bg-blue-50 px-2.5 py-1.5 text-[11px] font-semibold text-blue-700 hover:bg-blue-100 disabled:cursor-not-allowed disabled:opacity-60"
                          >
