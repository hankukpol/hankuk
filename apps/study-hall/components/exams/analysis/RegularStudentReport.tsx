...
export function RegularStudentReport({ report, mode }: { report: Report; mode: "admin" | "student" }) {
  ...
  return <div className="admin-flat-page">
    ...
    <section id="personal-subjects" className="admin-section"><h2 className="admin-section-title">과목별 비교</h2>
    ...
    <section className="admin-section"><h2 className="admin-section-title">과목 균형과 학습 조언</h2><div className="admin-notice"><p>{stats.balance.assessment} (표준편차 {value(stats.balance.stdDev)})</p>{stats.advice.length ? <ul>{stats.advice.map((advice, index) => <li key={index}>{advice}</li>)}</ul> : <p>추가 학습 조언이 없습니다.</p>}</div></section>
    ...
    <section id="personal-trend" className="admin-section"><h2 className="admin-section-title">날짜별 추이</h2>{hasTrend ? <ExamScoreChart results={trend} variant={mode === "admin" ? "admin" : "portal"} /> : <p className="admin-empty-state">직전 시험이 없어 비교할 수 없습니다. 다음 시험부터 표시됩니다.</p>}</section>
    <section className="admin-section"><h2 className="admin-section-title">목표 대비</h2>{report.target ? <p className="admin-notice">목표 {value(report.target.targetScore)}점 · 목표와 차이 {value(report.target.gap)}점 ({value(report.target.gapPercent)}%)</p> : <p className="admin-help">등록된 목표 점수가 없습니다.</p>}{report.flags.length > 0 && <ul className="admin-help">{report.flags.map((flag, index) => <li key={`${flag.kind}-${index}`}>{flag.detail}</li>)}</ul>}</section>
