...
import type { RegularCohortAnalysis as Analysis, RegularStudentReport as Report, RegularSessionListItem } from "@/lib/exam-analysis-types";
...
function StudentDetail({ url }: { url: string }) {
  ...
  return request.data ? <RegularStudentReport report={request.data.report} mode="admin" /> : <RequestStatus {...request} />;
}

function CohortSession({ base, examTypeId, examDate }: { base: string; examTypeId: string; examDate: string }) {
  ...
  return <div className="admin-flat-page">
    ...
    <section className="admin-section"><h2 className="admin-section-title">과목별 반·외부 비교</h2><div className="admin-table-frame"><table><thead><tr>{["과목", "반 평균", "외부 평균", "격차 (반 − 외부)", "취약 인원"].map((label) => <th scope="col" key={label}>{label}</th>)}</tr></thead><tbody>{analysis.subjects.map((subject) => {
    ...
    <section className="admin-section"><h2 className="admin-section-title">외부 성적 분포</h2><DistributionBars distribution={analysis.external.distribution} /></section>
    <section className="admin-section"><h2 className="admin-section-title">반 오답률 TOP10</h2>{analysis.classWrongTop.length === 0 ? <p className="admin-empty-state">{analysis.internal.count === 0 ? "매칭된 학생이 없습니다." : "집계할 문항 응답이 없습니다."}</p> : <div className="admin-table-frame"><table><thead><tr>{["과목", "번호", "정답", "반 정답률", "외부 정답률", "격차 (반 − 외부)"].map((label) => <th scope="col" key={label}>{label}</th>)}</tr></thead><tbody>{analysis.classWrongTop.map((item) => <tr key={`${item.subjectId}-${item.itemNo}`}><td className="admin-table-name">{item.subjectName}</td><td>{item.itemNo}</td><td>{item.answerKey}</td><td>{number(item.internalCorrectRatePct, "%")}</td><td>{number(item.externalCorrectRatePct, "%")}</td><td>{number(item.gap, "%p")}</td></tr>)}</tbody></table></div>}</section>
    <section className="admin-section"><h2 className="admin-section-title">반 석차표</h2><p className="admin-help">학생 이름을 선택하면 개인 분석을 엽니다. 직전 시험 대비 석차 상승은 ▲, 하락은 ▼로 표시합니다.</p>{analysis.ranking.length === 0 ? <p className="admin-empty-state">이 시험에 매칭된 반 학생이 없습니다.</p> : <div className="admin-table-frame"><table><thead><tr><th scope="col">반 석차</th><th scope="col">이름</th><th scope="col">총점</th>{analysis.subjects.map((subject) => <th scope="col" key={subject.id}>{subject.name}</th>)}<th scope="col">외부 상위%</th><th scope="col">직전 시험 총점</th><th scope="col">직전 시험 석차</th><th scope="col">표시</th></tr></thead><tbody>{analysis.ranking.map((row) => <tr key={row.studentId} onClick={() => setStudent({ id: row.studentId, name: row.name })}><td>{row.internalRank}등</td><td className="admin-table-name"><button type="button" className="admin-button admin-button-compact" onClick={() => setStudent({ id: row.studentId, name: row.name })}>{row.name}</button></td><td>{row.totalScore}</td>{analysis.subjects.map((subject) => <td key={subject.id}>{row.subjectScores[subject.id] ?? (subject.alternateGroup && analysis.subjects.some((other) => other.alternateGroup === subject.alternateGroup && row.subjectScores[other.id] != null) ? "선택 안 함" : "미응시")}</td>)}<td>{number(row.externalTopPercent, "%")}</td><td>{row.delta ? delta(row.delta.total) : "—"}</td><td>{row.delta ? delta(row.delta.rank) : "—"}</td><td>{row.flags.some((flag) => flag.kind === "totalDrop" || flag.kind === "rankDrop") && <span className="admin-badge">하락</span>} {row.isPartial && <span className="admin-badge">일부 미응시</span>}</td></tr>)}</tbody></table></div>}</section>
    <section className="admin-section"><h2 className="admin-section-title">학습 확인 대상</h2>{!hasPreviousExam ? <p className="admin-empty-state">비교할 직전 시험이 없습니다.</p> : analysis.declines.length ? <ul className="admin-help">{analysis.declines.map((row) => <li key={row.studentId}>{row.name}: {row.flags.map((flag) => flag.detail).join(" · ")}</li>)}</ul> : <p className="admin-help">학습 확인 신호가 없습니다.</p>}</section>
    <section className="admin-section"><h2 className="admin-section-title">일부 미응시 학생</h2>{analysis.partials.length ? <ul className="admin-help">{analysis.partials.map((row) => <li key={row.studentId}>{row.name}: {row.missingSubjects.join(", ")}</li>)}</ul> : <p className="admin-help">일부 미응시 학생이 없습니다.</p>}</section>
    <SlideOver open={student !== null} title={student ? `${student.name} 개인 분석` : "개인 분석"} onClose={() => setStudent(null)}>{student && <StudentDetail key={student.id} url={`${base}/student/${encodeURIComponent(student.id)}?${query}`} />}</SlideOver>
  </div>;
}

function SessionSelector({ base, examTypeId }: { base: string; examTypeId: string }) {
  const id = useId();
  ...
  return <div className="admin-flat-page"><div className="admin-filter-bar"><label className="admin-label" htmlFor={id}>시험 날짜</label><select id={id} value={selected.examDate} onChange={(event) => setExamDate(event.target.value)}>{sessions.map((session) => <option key={session.sessionId} value={session.examDate}>{session.examDate.slice(0, 10)} · 반 {session.participantCount}명</option>)}</select></div><CohortSession key={`${examTypeId}-${selected.examDate}`} base={base} examTypeId={examTypeId} examDate={selected.examDate} /></div>;
}

export function RegularCohortAnalysis({ divisionSlug, examTypes }: { divisionSlug: string; examTypes: ExamTypeItem[] }) {
  const id = useId();
  const [selectedId, setSelectedId] = useState(examTypes[0]?.id ?? "");
