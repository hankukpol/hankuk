...
## 현행 디자인 문서 (2026-09-05)

이 문서는 초기 기능·구조 계획이다. 현재 관리자 UI/UX 규격은 [DESIGN.md](../DESIGN.md), 구현 경로와 검증은 [로컬 디자인 구현 안내](../docs/DESIGN_IMPLEMENTATION.md)를 따른다.

관리자는 Pretendard, 테넌트 강조색, 흰색 한 페이지형 본문, 폴더형 1차 탭, 밑줄형 서브 탭을 사용한다. 표·탭·슬라이드 모달은 직각이며 일반 버튼·입력창·중앙 모달 외곽은 8px이다. 카드 UI에 관한 아래 학생 수강증 설명을 관리자 탭 본문에 확대 적용하지 않는다. 학생·직원·공개 QR의 기존 기능과 시각 범위는 분리해서 유지한다.

이번 갱신은 디자인 참조만 정리하며 아래 초기 개발 일정·데이터 모델·인프라 결정을 현재 검증 결과로 바꾸지 않는다.
