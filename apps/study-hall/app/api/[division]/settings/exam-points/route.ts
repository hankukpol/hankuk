...
import { getExamPointSettings, updateExamPointSettings } from "@/lib/services/exam-point-settings.service";
...
export async function PATCH(request:NextRequest,{params}:{params:{division:string}}) {
  ...
  try{return NextResponse.json({config:await updateExamPointSettings(params.division,parsed.data,auth.session)});}
