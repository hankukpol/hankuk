  ...
  return <section className="admin-section" id="exam-point-settings" aria-labelledby="exam-point-settings-title">
    ...
    {!form ? <div className="mt-4">{error ? <button type="button" className="admin-button admin-button-secondary" onClick={()=>setRevision(value=>value+1)}>다시 불러오기</button> : <p role="status" className="admin-help">설정을 불러오는 중입니다.</p>}</div> : <form onSubmit={save} className="mt-4 space-y-4">
      <fieldset disabled={saving} className="min-w-0 space-y-4">
        ...
        <label className="block min-w-0"><span className="admin-label mb-2 block">월간 성적 순위 기준</span><select className="w-full min-w-0" value={form.rankAggregation} onChange={e=>update("rankAggregation",e.target.value as ExamPointAutomation["rankAggregation"])}><option value="AVERAGE">응시한 시험의 월평균 점수</option><option value="TOTAL">월 누적 점수</option></select><span className="admin-help mt-2 block">100점 기준으로 환산해 아침·정기를 각각 집계합니다. 순위 상점은 다음 달에 월 1회 지급합니다. 미응시 벌점은 별도로 계산합니다.</span></label>
