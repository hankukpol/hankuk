...
import type { SpecialDivisionGroup, SpecialRegistrationWithScore } from "./page";
...
function initRows(registrations: SpecialRegistrationWithScore[]): ScoreRow[] {
...
export function SpecialScoreEntryClient({
  eventId,
  divisionGroups,
}: {
  eventId: string;
  divisionGroups: SpecialDivisionGroup[];
}) {
  ...
  const handleSave = async (division: string) => {
    ...
    try {
      // Reuse the monthly exam scores API — it works for any ExamRegistration
