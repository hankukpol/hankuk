...
### 전체 표의 텍스트 표시 (2026-09-15 사용자 확정)

관리자·조교·학생 및 모달의 모든 표에서 상태·경고·직렬·분류 배지는 배경,
테두리, 둥근 모서리, 그림자 없이 텍스트로 표시한다. 업무 의미의 글자색은 유지한다.
이 규칙은 아래 학생 포털 한정 배지 규격보다 우선한다. 표 밖 배지에는 적용하지 않는다.
행의 상세 보기·수정·삭제 액션도 외곽 상자를 없애되 클릭 영역, 아이콘과 키보드
포커스는 유지한다. 입력·선택창·체크박스 등 실제 편집 컨트롤은 유지한다.
...
진도별 학습 점검처럼 비교 항목이 10열인 표는 1280px 이상에서 진도·판단·다음 학습 열에 더 넓은 폭을 배분하고 한글 어절을 유지한다. 768~1279px에서는 진도명을 전체 폭으로 놓고 항목명과 값을 4열로, 768px 미만에서는 기존 2열로 배치한다. 다음 학습은 마지막 전체 폭 행이다. 8px 셀 여백, 기존 표·글자·색 토큰을 재사용하고 각 수치는 독립 셀로 유지한다.
...
관리자 모바일 성적표는 이름 88px, 수험번호 104px, 조회 점수 80px, 입력 점수 96px, 비고 160px를 기준으로 열을 배치한다. 적용 클래스는 `admin-weekly-score-table`과 `admin-score-entry-table`이다. 셀 여백은 8px이며 이름과 긴 과목명은 줄바꿈한다. 첫 식별 열 고정과 표 내부 가로 스크롤을 유지하고, 데스크톱 표 규격은 바꾸지 않는다.
