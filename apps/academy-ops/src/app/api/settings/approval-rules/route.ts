...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    if (
      !Number.isInteger(refundApprovalThreshold) || refundApprovalThreshold < 0 ||
      !Number.isInteger(discountApprovalThreshold) || discountApprovalThreshold < 0 ||
      !Number.isInteger(cashApprovalThreshold) || cashApprovalThreshold < 0
    ) {
      return NextResponse.json(
