...
이 문서는 교재 구매·배정·수령 기능의 개발 명세다. 현재 시각 규격은 [DESIGN.md](../DESIGN.md)가 우선한다. 아래 탭/화면 설명은 기능 구분이며 박스·카드·폴더 형태를 별도로 지시하지 않는다.

- 관리자 1차 메뉴만 폴더형, 교재 배정·교재 수령현황 등 2차 메뉴는 직선 밑줄형이다.
- 탭 본문은 카드가 중첩되지 않는 한 페이지형 섹션이다. 표·탭·슬라이드 모달은 직각을 유지한다.
- 등록·배정·확인창은 공통 관리자 폰트와 모달 구조를 쓴다. 자세한 구현 경로는 [디자인 구현 안내](DESIGN_IMPLEMENTATION.md)를 따른다.
- 이 문서의 기존 DB/API/업무 조건은 디자인 문서 갱신으로 변경하지 않는다.
