  ...
  return (
    ...
    >
      ...
      <SectionCard
        ...
        description="조원 목록, 공지, 댓글 기록을 이 화면에서 바로 확인할 수 있습니다."
        action={
        ...
        <div className="rounded-[10px] border border-slate-200 bg-slate-50 px-4 py-4 text-sm text-slate-600">
          ...
          {!isLoading && isApplyClosed && !isRoomClosed ? (
            <p className="mt-3 text-xs text-amber-700">
              지원 마감 후에는 새 댓글 확인과 공지 확인만 권장합니다.
            </p>
          ) : null}
      ...
      <SectionCard
        title="댓글"
        description="실시간 채팅 대신 기록형 댓글과 공지를 이 화면에서 확인합니다."
        action={
          <button
            type="button"
      ...
      >
        <div className="space-y-3">
          ...
          <div className="rounded-[10px] border border-slate-200 bg-slate-50 px-4 py-4 text-xs leading-5 text-slate-500">
            댓글은 자동 갱신되지 않습니다. 다른 조원의 새 댓글이나 관리자 공지는
            `새로고침`으로 확인하세요.
          </div>
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            <textarea
              ...
              rows={3}
              maxLength={500}
              className="w-full rounded-[10px] border border-slate-200 bg-white px-3 py-3 text-sm leading-6"
              placeholder="조원에게 남길 댓글이나 확인 메모를 입력해 주세요."
            />
            <button
              type="button"
