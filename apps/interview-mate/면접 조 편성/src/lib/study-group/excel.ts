...
export function downloadCsvTemplate(): void {
  const header = '이름,연락처,성별,직렬,지역,나이,필기성적,조';
  const example = '홍길동,010-1234-5678,남,일반,서울,28,85.5,';
...
export async function exportGroupsToExcel(
  ...
  summarySheet.columns = createSheetColumns(hasAges, hasScores, false);
  ...
  for (const group of groups) {
    ...
    groupSheet.columns = createSheetColumns(hasAges, hasScores, true);
