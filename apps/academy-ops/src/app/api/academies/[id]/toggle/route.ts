...
import { toggleAcademyActive } from "@/lib/super-admin";
...
export async function PATCH(
  ...
  try {
    const academy = await toggleAcademyActive(academyId, {
      adminId: auth.context.adminUser.id,
      ipAddress: request.headers.get("x-forwarded-for"),
    });
    return NextResponse.json({ data: academy });
  } catch (error) {
    const message = error instanceof Error ? error.message : "지점 상태 변경 중 오류가 발생했습니다.";
