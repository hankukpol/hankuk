...
type AddMemoFormProps = {
  examNumber: string;
  currentAdminName: string;
  onAdded: (memo: AdminMemoRow) => void;
  onCancel: () => void;
};

function AddMemoForm({ examNumber, currentAdminName, onAdded, onCancel }: AddMemoFormProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error((body as { error?: string }).error ?? "메모 저장에 실패했습니다.");
      }
      const body = await res.json() as { data: AdminMemoRow };
      onAdded(body.data);
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      <div className="space-y-4">
        <div>
          <label className="mb-1 block text-xs font-semibold text-slate">제목 *</label>
          <input
            type="text"
            value={title}
        ...
        <div className="flex flex-wrap gap-4">
          <div className="flex-1 min-w-[140px]">
            ...
            >
              {Object.entries(MEMO_COLOR_LABEL).map(([k, v]) => (
          ...
          <div className="flex-1 min-w-[140px]">
            ...
            <select
              ...
              className="w-full rounded-2xl border border-ink/10 bg-white px-4 py-3 text-sm"
            >
              {Object.entries(MEMO_SCOPE_LABEL).map(([k, v]) => (
                <option key={k} value={k}>
                  {v}
                </option>
              ))}
...
export function StudentAdminMemos({ examNumber, initialMemos, currentAdminId, currentAdminName }: Props) {
  const [memos, setMemos] = useState<AdminMemoRow[]>(initialMemos);
  const [showAddForm, setShowAddForm] = useState(false);
  const [deletingId, setDeletingId] = useState<number | null>(null);

  function handleAdded(memo: AdminMemoRow) {
  ...
  const activeMemos = memos.filter((m) => m.status !== "DONE");
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-slate">
          관리자 메모{" "}
          <span className="font-semibold text-ink">{memos.length}건</span>
          {activeMemos.length > 0 && (
            <span className="ml-2 text-xs text-ember">({activeMemos.length}건 진행중)</span>
          )}
        </p>
        {!showAddForm && (
          <button
      ...
      {/* 진행중 메모 */}
      {activeMemos.length === 0 && !showAddForm ? (
      ...
      {doneMemos.length > 0 && (
        <details className="group">
          <summary className="cursor-pointer select-none text-sm font-semibold text-slate transition hover:text-ink">
            완료된 메모 {doneMemos.length}건
          </summary>
...
function MemoCard({ memo, currentAdminId, deletingId, onDelete, onStatusChange }: MemoCardProps) {
  ...
  return (
    <div
      className={`rounded-[24px] border p-5 ${MEMO_COLOR_BG[memo.color]} relative`}
    >
      {memo.isPinned && (
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 flex-1">
          <p className="font-semibold text-ink leading-snug">{memo.title}</p>
          {memo.content && (
            <p className="mt-1.5 text-sm text-slate whitespace-pre-wrap leading-relaxed">
              {memo.content}
            </p>
          )}
        </div>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2">
        ...
        {memo.dueAt && (
          <span className="text-xs text-slate">마감: {formatDate(memo.dueAt)}</span>
        )}
        <span className="ml-auto text-xs text-slate">
          {memo.owner.name} · {formatDate(memo.createdAt)}
        </span>
      </div>
      {/* 상태 변경 + 삭제 */}
