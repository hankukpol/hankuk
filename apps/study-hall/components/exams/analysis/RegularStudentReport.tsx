...
export type { ReportSection } from "./PersonalReportTabs";

const value = (number: number | null | undefined, suffix = "") => number == null || !Number.isFinite(number) ? "집계 불가" : `${Number(number.toFixed(1))}${suffix}`;

export function RegularStudentReport({ report, mode, section, records }: { report: Report; mode: "admin" | "student"; section?: ReportSection; records?: ReactNode }) {
  ...
  return <div className="admin-flat-page" data-report-root>
    ...
    {history && <section data-report-section="trend" className="admin-section"><h2 className="admin-section-title">최근 6개월 개인 성적</h2>
      ...
      <div className="admin-table-frame"><table><thead><tr><th scope="col">시험일 / 월</th><th scope="col">총점 / 만점</th>{report.subjects.map(s => <th scope="col" key={s.id}>{s.name}</th>)}<th scope="col">반 석차</th><th scope="col">외부 석차</th><th scope="col">외부 상위%</th><th scope="col">응시 상태</th></tr></thead><tbody>{history.months.flatMap(month => {
        ...
        return rows.map(row => <tr key={row.date}><th scope="row">{row.date}</th><td>{value(row.total)} / {value(row.fullScore)}</td>{report.subjects.map(s => <td key={s.id}>{value(row.subjectScores[s.id])}</td>)}<td>{row.internalRank}등</td><td>{value(row.externalRank, "등")}</td><td>{value(row.externalTopPercent, "%")}</td><td>{row.isPartial ? "일부 미응시" : "응시"}</td></tr>);
      })}</tbody></table></div>
