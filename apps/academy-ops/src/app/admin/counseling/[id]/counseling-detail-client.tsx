...
export function CounselingDetailClient({ record: initialRecord }: Props) {
  ...
  async function requestJson(url: string, init?: RequestInit) {
    ...
    if (!response.ok) {
      throw new Error((payload.error as string | undefined) ?? "요청에 실패했습니다.");
    }
    return payload;
  }
  ...
  function handleSave() {
    ...
    startTransition(async () => {
      try {
        const { record: updated } = await requestJson(`/api/counseling/${record.id}`, {
          method: "PUT",
          body: JSON.stringify({
        ...
        setRecord({
          ...r,
          counseledAt: typeof r.counseledAt === "string" ? r.counseledAt : new Date(r.counseledAt as unknown as string).toISOString(),
  ...
  function handleDelete() {
    ...
    startTransition(async () => {
      try {
        await requestJson(`/api/counseling/${record.id}`, { method: "DELETE" });
    ...
    <div className="space-y-6">
      ...
      {showDeleteConfirm ? (
        <div className="rounded-[28px] border border-red-200 bg-red-50 p-5">
          <p className="font-semibold text-red-800">이 면담 기록을 삭제하시겠습니까?</p>
          <p className="mt-1 text-sm text-red-700">삭제 후에는 되돌릴 수 없으며 통계에도 반영됩니다.</p>
        ...
        {isEditing ? (
          <form ref={formRef} className="p-6 space-y-5">
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              ...
              <div className="xl:col-span-2">
                <label className="mb-2 block text-sm font-medium">다음 면담 예정일</label>
                <input
                  type="date"
                  name="nextSchedule"
                  defaultValue={toDateInputValue(record.nextSchedule ?? undefined)}
                  className="w-full rounded-2xl border border-ink/10 px-4 py-3 text-sm focus:border-ember/50 focus:outline-none"
                />
              </div>
            </div>
          ...
          <dl className="divide-y divide-ink/10">
            <div className="grid grid-cols-[180px_1fr] gap-4 px-6 py-4">
              <dt className="text-sm font-medium text-slate self-start pt-0.5">담당 강사</dt>
              <dd className="text-sm text-ink">{record.counselorName}</dd>
            </div>
            <div className="grid grid-cols-[180px_1fr] gap-4 px-6 py-4">
              <dt className="text-sm font-medium text-slate self-start pt-0.5">면담 일시</dt>
            ...
            <div className="grid grid-cols-[180px_1fr] gap-4 px-6 py-4">
              <dt className="text-sm font-medium text-slate self-start pt-0.5">다음 면담 예정일</dt>
              <dd className="text-sm text-ink">
                {record.nextSchedule
            ...
            <div className="grid grid-cols-[180px_1fr] gap-4 px-6 py-4">
              <dt className="text-sm font-medium text-slate self-start pt-0.5">면담 내용</dt>
              <dd className="whitespace-pre-wrap text-sm text-ink leading-7">{record.content}</dd>
            </div>
            {record.recommendation ? (
              <div className="grid grid-cols-[180px_1fr] gap-4 px-6 py-4">
                <dt className="text-sm font-medium text-slate self-start pt-0.5">추천 학습 방향</dt>
                <dd className="whitespace-pre-wrap text-sm text-ink leading-7">{record.recommendation}</dd>
              </div>
            ) : null}
          </dl>
        )}
