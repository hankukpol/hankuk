...
begin
  ...
  insert into class_pass.enrollment_payments (
    ...
    nullif(p_payment->>'bankAccountLast4', ''),
  ...
  -- Recalculate tuition net after both the refund and the new payment
  select
  ...
  -- Set enrollment to refunded when all tuition is net-refunded after correction
  ...
  -- Reactivate enrollment when correction brings net back to positive
  update class_pass.enrollments
  set status = 'active',
      refunded_at = null
  where id = p_enrollment_id
