...
export async function closeDivisionExamPoints(slug: string) {
  ...
  if (!config.enabled || !config.effectiveFrom) return {grantedCount:0,revokedCount:0,leaveCount:0};
  const today = kstDate();
  ...
  const leave = config.settleUnusedLeaveAutomatically && lastDay >= config.effectiveFrom
