...
> 이 파일은 AI 코딩 에이전트(Codex, Codex 등)가 이 프로젝트를 올바르게 개발하기 위한 핵심 지침서입니다.
...
- 사용 여부, 적용일, 대상 요일·교시, 규칙과 점수, 한도, 임계값, 자동 부여·확정 방식은 각 학원의 설정에서 독립적으로 관리한다. 새 기능에는 해당 학원 관리자가 설정할 수 있는 경로를 함께 제공한다.
- 계산·조회·저장·자동 마감은 현재 학원의 division_id와 설정을 사용한다. 다른 학원의 규칙 ID 연결 및 데이터 참조를 거부한다.
- 공통 코드 배포와 특정 학원 운영 활성화를 구분한다. 이번 활성화 대상은 경찰파트이며 다른 학원의 기존 설정·데이터는 명시적 요청 없이 변경하지 않는다. 새 자동화는 기본 비활성이다.
- 서로 다른 두 학원의 설정·점수·요일로 독립 계산을 검증하고, 타 학원 접근 차단 및 비활성 학원 보존을 확인한다.
- 아침모의고사의 시작일은 학원 관리자가 달력으로 지정한다. 시험 요일은 월~금 중 체크한 요일만 사용한다. 시작일 이전·미선택 요일·등록한 휴강일은 미응시 벌점과 월 개근 계산에서 제외하고, 시작일 미지정 상태에서는 아침모의고사 자동 부여를 하지 않는다.
- 성적 순위 상점은 일별·회차별로 지급하지 않는다(운영자 정정, 2026-09-14). 아침·정기를 분리해 월 성적을 집계하고 월 종료 후 각 월 1회 지급한다. 월평균/누적 기준과 지급 규칙은 학원별 설정으로 관리한다. 첫 시험·진행 중인 달·설정 재저장 때 순위 상점을 미리 부여하지 않는다.
- 경찰 전용 초기화 스크립트와 실제 규정 자료는 경찰에 한정할 수 있지만 공통 서비스의 계산 근거로 하드코딩하지 않는다.
