'use client'

import Image from 'next/image'
import Link from 'next/link'
import { useParams } from 'next/navigation'
import { useCallback, useEffect, useMemo, useRef, useState } from 'react'
import { useTenantConfig } from '@/components/TenantProvider'
import type { Course, Enrollment } from '@/types/database'
import { withTenantPrefix } from '@/lib/tenant'

export type PhotosPageData = {
  course: Course
  enrollments: Enrollment[]
}

type MatchResult = {
  fileName: string
  file: File
  examNumber: string
  enrollment: Enrollment | null
  preview: string
}

type BulkPhotoUploadPageProps = {
  initialData?: PhotosPageData | null
  initialError?: string
  initialLoaded?: boolean
}

function revokeMatches(matches: MatchResult[]) {
  for (const match of matches) {
    URL.revokeObjectURL(match.preview)
  }
}

export default function BulkPhotoUploadPage({
  initialData = null,
  initialError = '',
  initialLoaded = Boolean(initialData),
}: BulkPhotoUploadPageProps) {
  const params = useParams<{ id: string }>()
  const tenant = useTenantConfig()
  const courseId = Number(params.id)
  const dropRef = useRef<HTMLDivElement>(null)

  const [course, setCourse] = useState<Course | null>(initialData?.course ?? null)
  const [enrollments, setEnrollments] = useState<Enrollment[]>(initialData?.enrollments ?? [])
  const [matches, setMatches] = useState<MatchResult[]>([])
  const [loading, setLoading] = useState(!initialLoaded)
  const [uploading, setUploading] = useState(false)
  const [progress, setProgress] = useState({ done: 0, total: 0 })
  const [error, setError] = useState(initialError)
  const [message, setMessage] = useState('')
  const [dragOver, setDragOver] = useState(false)

  useEffect(() => {
    return () => {
      revokeMatches(matches)
    }
  }, [matches])

  useEffect(() => {
    if (!Number.isInteger(courseId) || courseId <= 0) {
      setError('잘못된 강의 ID입니다.')
      setLoading(false)
      return
    }

    if (initialLoaded) {
      return
    }

    Promise.all([
      fetch(`/api/courses/${courseId}`, { cache: 'no-store' }).then((response) => response.json()),
      fetch(`/api/enrollments?courseId=${courseId}`, { cache: 'no-store' }).then((response) => response.json()),
    ])
      .then(([coursePayload, enrollmentPayload]) => {
        setCourse(coursePayload.course as Course)
        setEnrollments((enrollmentPayload.enrollments ?? []) as Enrollment[])
      })
      .catch(() => setError('데이터를 불러오지 못했습니다.'))
      .finally(() => setLoading(false))
  }, [courseId, initialLoaded])

  const enrollmentByExam = useMemo(() => {
    const map = new Map<string, Enrollment>()
    for (const enrollment of enrollments) {
      if (enrollment.exam_number) {
        map.set(enrollment.exam_number.trim(), enrollment)
      }
    }
    return map
  }, [enrollments])

  const processFiles = useCallback((files: FileList | File[]) => {
    const nextResults: MatchResult[] = []
    const fileArray = Array.from(files).filter((file) => file.type.startsWith('image/'))

    for (const file of fileArray) {
      const baseName = file.name.replace(/\.[^.]+$/, '').trim()
      const enrollment = enrollmentByExam.get(baseName) ?? null

      nextResults.push({
        fileName: file.name,
        file,
        examNumber: baseName,
        enrollment,
        preview: URL.createObjectURL(file),
      })
    }

    nextResults.sort((left, right) => {
      if (left.enrollment && !right.enrollment) return -1
      if (!left.enrollment && right.enrollment) return 1
      return left.examNumber.localeCompare(right.examNumber, 'ko-KR', { numeric: true })
    })

    setMatches((current) => {
      revokeMatches(current)
      return nextResults
    })
    setError('')
    setMessage('')
  }, [enrollmentByExam])

  function handleFileChange(event: React.ChangeEvent<HTMLInputElement>) {
    if (event.target.files?.length) {
      processFiles(event.target.files)
    }
  }

  function handleDrop(event: React.DragEvent) {
    event.preventDefault()
    setDragOver(false)
    if (event.dataTransfer.files.length) {
      processFiles(event.dataTransfer.files)
    }
  }

  async function handleUpload() {
    const matched = matches.filter((match) => match.enrollment)
    if (matched.length === 0) {
      setError('업로드할 매칭 사진이 없습니다.')
      return
    }

    setUploading(true)
    setProgress({ done: 0, total: matched.length })
    setError('')
    let successCount = 0

    for (const match of matched) {
      const formData = new FormData()
      formData.append('photo', match.file)

      const response = await fetch(`/api/enrollments/${match.enrollment!.id}/photo`, {
        method: 'POST',
        body: formData,
      })

      if (response.ok) {
        successCount += 1
      }

      setProgress((current) => ({ ...current, done: current.done + 1 }))
    }

    setUploading(false)
    setMessage(`${successCount}/${matched.length}건 업로드 완료`)
    setMatches((current) => {
      revokeMatches(current)
      return []
    })
  }

  const matchedCount = matches.filter((match) => match.enrollment).length
  const unmatchedCount = matches.length - matchedCount

  if (loading) {
    return <p className="py-12 text-center text-sm text-gray-400">불러오는 중...</p>
  }

  if (!course) {
    return <p className="py-12 text-center text-sm text-red-500">{error || '강의를 찾을 수 없습니다.'}</p>
  }

  return (
    <div className="flex flex-col gap-6">
      <div>
        <Link
          href={withTenantPrefix(`/dashboard/courses/${courseId}/students`, tenant.type)}
          className="text-xs font-medium text-gray-400 hover:underline"
        >
          수강생 관리
        </Link>
        <h2 className="mt-1 text-xl font-extrabold text-gray-900">사진 일괄 업로드</h2>
        <p className="mt-1 text-sm text-gray-400">
          파일명을 수험번호로 맞춰 주세요. 예: <span className="font-semibold text-gray-600">26809.jpg</span>
        </p>
      </div>

      <div
        ref={dropRef}
        onDragOver={(event) => {
          event.preventDefault()
          setDragOver(true)
        }}
        onDragLeave={() => setDragOver(false)}
        onDrop={handleDrop}
        className={`flex flex-col items-center justify-center rounded-2xl border-2 border-dashed px-6 py-12 text-center transition ${
          dragOver ? 'border-blue-400 bg-blue-50' : 'border-slate-200 bg-white'
        }`}
      >
        <p className="text-sm font-semibold text-gray-700">사진 파일을 드래그하거나 클릭해서 선택하세요.</p>
        <p className="mt-1 text-xs text-gray-400">JPEG, PNG, WebP / 2MB 이하 / 파일명 = 수험번호</p>
        <label className="mt-4 cursor-pointer rounded-xl bg-blue-600 px-5 py-2.5 text-sm font-bold text-white hover:bg-blue-700">
          파일 선택
          <input
            type="file"
            accept="image/jpeg,image/png,image/webp"
            multiple
            className="hidden"
            onChange={handleFileChange}
          />
        </label>
      </div>

      {error && <p className="text-xs text-red-500">{error}</p>}
      {message && <p className="text-xs text-emerald-600">{message}</p>}

      {matches.length > 0 && (
        <div className="rounded-2xl bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold text-gray-700">
              매칭 결과
              <span className="ml-2 text-xs font-normal text-gray-400">
                매칭 {matchedCount}건 / 미매칭 {unmatchedCount}건
              </span>
            </h3>
            <div className="flex gap-2">
              <button
                type="button"
                onClick={() => {
                  setMatches((current) => {
                    revokeMatches(current)
                    return []
                  })
                }}
                className="rounded-lg bg-slate-100 px-3 py-1.5 text-xs font-semibold text-slate-600 hover:bg-slate-200"
              >
                초기화
              </button>
              <button
                type="button"
                onClick={() => void handleUpload()}
                disabled={uploading || matchedCount === 0}
                className="rounded-lg bg-blue-600 px-4 py-1.5 text-xs font-bold text-white hover:bg-blue-700 disabled:opacity-50"
              >
                {uploading ? `업로드 중 (${progress.done}/${progress.total})` : `매칭된 ${matchedCount}건 업로드`}
              </button>
            </div>
          </div>

          <div className="mt-4 grid gap-2">
            {matches.map((match) => (
              <div
                key={match.fileName}
                className={`flex items-center gap-3 rounded-xl border px-4 py-3 ${
                  match.enrollment ? 'border-emerald-200 bg-emerald-50/50' : 'border-red-200 bg-red-50/50'
                }`}
              >
                <Image
                  src={match.preview}
                  alt=""
                  width={36}
                  height={48}
                  unoptimized
                  className="h-12 w-9 shrink-0 rounded object-cover"
                />
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-semibold text-gray-900">{match.fileName}</p>
                  <p className="text-xs text-gray-500">수험번호: {match.examNumber}</p>
                </div>
                {match.enrollment ? (
                  <span className="shrink-0 text-xs font-semibold text-emerald-700">
                    매칭: {match.enrollment.name}
                  </span>
                ) : (
                  <span className="shrink-0 text-xs font-semibold text-red-500">매칭 실패</span>
                )}
              </div>
            ))}
          </div>
        </div>
      )}
    </div>
  )
}
