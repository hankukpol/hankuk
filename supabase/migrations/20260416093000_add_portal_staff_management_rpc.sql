...
begin
  case p_app_key
    ...
    when 'study-hall' then
      ...
      if p_role_key = 'super_admin' then
        insert into study_hall.admins (user_id, name, role, division_id, is_active)
        values (p_user_id, coalesce(p_full_name, coalesce(p_email, '운영자')), 'SUPER_ADMIN', null, true)
        ...
        insert into study_hall.admins (user_id, name, role, division_id, is_active)
        values (p_user_id, coalesce(p_full_name, coalesce(p_email, '운영자')), 'ASSISTANT', v_division_id, true)
        on conflict (user_id) do update
        set
          name = excluded.name,
          role = excluded.role,
        ...
        insert into study_hall.admins (user_id, name, role, division_id, is_active)
        values (p_user_id, coalesce(p_full_name, coalesce(p_email, '운영자')), 'ADMIN', v_division_id, true)
        on conflict (user_id) do update
        set
          name = excluded.name,
          role = excluded.role,
...
create or replace function public.deactivate_staff_for_app(
  p_user_id uuid,
  p_app_key text
) returns jsonb
language plpgsql
...
as $$
declare
  v_operator_account_ids bigint[];
begin
  case p_app_key
    when 'academy-ops' then
      update academy_ops.admin_users
      set
        is_active = false,
        updated_at = timezone('utc', now())
      where id = p_user_id;
    ...
    when 'study-hall' then
      update study_hall.admins
      set is_active = false
      where user_id = p_user_id;

    when 'class-pass' then
      select array_agg(id)
      into v_operator_account_ids
      from class_pass.operator_accounts
      where shared_user_id = p_user_id;
      ...
      if v_operator_account_ids is not null then
        update class_pass.operator_memberships
        set
          is_active = false,
          updated_at = timezone('utc', now())
        where operator_account_id = any(v_operator_account_ids);
      end if;
    ...
    when 'interview-pass' then
      ...
      set
        status = case
          when status = 'claimed' then 'revoked'
          else status
        end,
        updated_at = timezone('utc', now())
  ...
  return jsonb_build_object('ok', true, 'appKey', p_app_key);
end;
$$;

revoke all on function public.provision_staff_for_app(uuid, text, text, text, text, text) from public;
grant execute on function public.provision_staff_for_app(uuid, text, text, text, text, text) to service_role;

revoke all on function public.deactivate_staff_for_app(uuid, text) from public;
grant execute on function public.deactivate_staff_for_app(uuid, text) to service_role;
