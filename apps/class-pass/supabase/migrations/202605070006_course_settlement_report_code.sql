alter table class_pass.courses
  add column if not exists settlement_report_code text;

alter table class_pass.courses
  drop constraint if exists class_pass_courses_settlement_report_code_check;

alter table class_pass.courses
  add constraint class_pass_courses_settlement_report_code_check
  check (
    settlement_report_code is null
    or length(trim(settlement_report_code)) <= 20
  );
