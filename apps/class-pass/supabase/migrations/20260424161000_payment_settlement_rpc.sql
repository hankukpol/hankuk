create or replace function class_pass.get_payment_settlement(
  p_from_date date,
  p_to_date date,
...
as $$
  with payment_agg as (
    ...
    group by p.paid_date, p.course_id, c.name, p.method, p.category
  ),
  refund_agg as (
    select
    ...
    where p.status <> 'voided'
      ...
      and (p_course_id is null or p.course_id = p_course_id)
      and (p_division is null or c.division = p_division)
    group by (r.refunded_at at time zone 'Asia/Seoul')::date, p.course_id, c.name, p.method, p.category
  )
  select
  ...
  order by paid_date desc, course_name asc, method asc, category asc;
$$;
