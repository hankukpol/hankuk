  ...
  return (
    <div className="admin-flat-page">
      ...
      <section className="admin-section">
        {/* 제목은 페이지 상단 h1이 담당한다. 여기서는 조회 조건과 저장만 둔다. */}
            ...
            <tbody>
              {filteredStudents.length === 0 ? (
                <tr>
                  <td
                    ...
                    className="admin-help px-4 py-12 text-center"
                  >
                    검색 조건에 맞는 학생이 없습니다.
                  </td>
                </tr>
              ...
              {filteredStudents.map((student) => (
                <tr key={student.id} className="align-top">
                  <td className="admin-table-name sticky left-0 z-10 w-[200px] min-w-[200px]">
                    <button
                      type="button"
          ...
          <div className="space-y-5">
            <div className="grid grid-cols-2 gap-1 rounded-lg bg-slate-100 p-1">
              {[
                ...
                <button
                  ...
                  className="admin-choice-button" data-active={activeBulkApplyDraft.mode === option.mode} aria-pressed={activeBulkApplyDraft.mode === option.mode}
                >
                  {option.label}
                </button>
              ))}
            ...
            {activeBulkApplyDraft.mode === "daily" && (
              <div className="space-y-2 border-t border-slate-100 pt-4">
                <p className="admin-label">오늘 전체 교시</p>
                ...
                <div className="flex flex-col gap-1 rounded-lg border border-slate-200 bg-white p-1 sm:flex-row">
                  {[
                    ...
                    <button
                      ...
                      className="admin-choice-button" data-active={activeBulkApplyDraft.overwriteExisting === option.overwriteExisting} aria-pressed={activeBulkApplyDraft.overwriteExisting === option.overwriteExisting}
                    >
                      {option.label}
                    </button>
                  ))}
