        ...
        <table className="data-table min-w-[640px] w-full">
          <thead>
            <tr>
              <th className="">구분</th>
              {columns.map((column) => (
                <th key={column.key} className="border-b border-slate-200 px-4 py-3 text-center">
          ...
          <tbody>
            ...
            {showMyScore ? (
              <tr>
                <td className="bg-service-50 font-semibold text-service-700">내 점수</td>
                {columns.map((column) => (
                  <td key={`${column.key}-myscore`} className="border-b border-slate-200 bg-service-50 px-4 py-3 text-center font-semibold tabular-nums text-service-700">
                    {myScore.toFixed(2)}
                  </td>
                ))}
              </tr>
            ...
            {scoreRows.map((row) => (
              <tr key={row.label}>
                <td className="bg-slate-50 font-medium text-slate-700">{row.label}</td>
                {columns.map((column, colIndex) => {
                  ...
                  return (
                    <td key={`${column.key}-${row.label}`} className="border-b border-slate-200 px-4 py-3 text-center tabular-nums">
                      {displayValue}
            ...
            {/* 참여자 행 */}
            <tr>
              <td className="bg-slate-50 font-medium text-slate-700">참여자</td>
              {columns.map((column) => (
                <td key={`${column.key}-participant`} className="border-b border-slate-200 px-4 py-3 text-center tabular-nums">
            ...
            {/* 응시인원 행 */}
            <tr>
              <td className="bg-slate-50 font-medium text-slate-700">응시인원</td>
              {columns.map((column) => (
                <td key={`${column.key}-applicant`} className="border-b border-slate-200 px-4 py-3 text-center tabular-nums">
                  {column.snapshot
