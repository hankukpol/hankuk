...
export function StudentAnalysisPage({ division, studentId, examTypes, initial }: { division: string; studentId: string; examTypes: ExamTypeItem[]; initial: Selection }) {
 ...
 return <div className="admin-flat-page">
  ...
  {!selected?<p className="admin-empty-state">등록된 시험 종류가 없습니다.</p>:current?.error?<div role="alert" className="admin-notice"><p>{current.error}</p><button className="admin-button" onClick={()=>setAttempt(value=>value+1)}>다시 시도</button></div>:!report?<p role="status">개인 성적을 불러오는 중입니다.</p>:current?.regular?<RegularStudentReport report={current.regular} mode="admin"/>:current?.morning?<MorningStudentReport report={current.morning} mode="admin"/>:null}
 </div>;
}
