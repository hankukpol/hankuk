...
type DiscountCodeEditFormProps = {
  discountCode: DiscountCodeData;
};

export function DiscountCodeEditForm({ discountCode }: DiscountCodeEditFormProps) {
  ...
  const [form, setForm] = useState<FormState>({
    ...
    discountValue: String(discountCode.discountValue),
    maxUsage: discountCode.maxUsage != null ? String(discountCode.maxUsage) : "",
    validFrom: discountCode.validFrom.slice(0, 10),
    validUntil: discountCode.validUntil ? discountCode.validUntil.slice(0, 10) : "",
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      try {
        ...
        const response = await fetch(`/api/settings/discount-codes/${discountCode.id}`, {
          method: "PATCH",
        ...
        if (!response.ok) {
          throw new Error(payload.error ?? "할인 코드 수정에 실패했습니다.");
        }

        toast.success("할인 코드를 수정했습니다.");
        router.push(`/admin/settings/discount-codes/${discountCode.id}`);
        router.refresh();
      } catch (error) {
        setErrorMessage(
  ...
  const codeAlreadyUsed = discountCode.usageCount > 0;
    ...
    <form onSubmit={handleSubmit} className="mt-8 max-w-2xl space-y-6">
      ...
      {codeAlreadyUsed && (
        <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
          <p className="font-semibold">사용된 코드</p>
          <p className="mt-0.5">
            이 코드는 이미 {discountCode.usageCount}회 사용되었습니다.
            코드 문자열과 할인 방식 변경은 신중히 하세요.
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div>
          ...
          <input
            type="number"
            min={codeAlreadyUsed ? discountCode.usageCount : 1}
            value={form.maxUsage}
          ...
          />
          {codeAlreadyUsed && (
            <p className="mt-1 text-xs text-slate">최솟값: 현재 사용 횟수({discountCode.usageCount}회) 이상</p>
          )}
        </div>
        <div>
          <label className="mb-1.5 block text-sm font-medium text-ink">
      ...
      <div className="flex items-center gap-3 pt-2">
        ...
        <a
          href={`/admin/settings/discount-codes/${discountCode.id}`}
