  ...
  return (
    <section className="rounded-xl border border-slate-200 bg-white p-5 sm:p-6">
      ...
      <div className="mt-4 overflow-x-auto">
        <table className="data-table min-w-[640px] w-full text-sm">
          <thead>
          ...
          <tbody>
            ...
            {showMyScore ? (
              <tr>
                <td className="border border-slate-200 bg-service-50 px-3 py-2 font-semibold text-service-700">내 점수</td>
                {columns.map((column) => (
                  <td key={`${column.key}-myscore`} className="border border-slate-200 bg-service-50 px-3 py-2 text-center font-semibold text-service-700">
                    {myScore.toFixed(2)}
                  </td>
                ))}
              </tr>
