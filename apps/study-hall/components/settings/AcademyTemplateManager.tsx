...
import { ApplicationDateCorrection } from "@/components/settings/ApplicationDateCorrection";
...
export function AcademyTemplateManager({ divisionSlug }: { divisionSlug: string }) {
  ...
  return <div className="admin-flat-page">
    ...
    <section className="admin-section"><h2 className="admin-section-title">적용 이력·예약</h2>{!library.applications.length?<p className="admin-help">적용 이력이 없습니다.</p>:<div className="admin-table-frame"><table className="admin-table"><thead><tr><th>적용일</th><th>템플릿</th><th>상태</th><th>요청자</th><th>관리</th></tr></thead><tbody>{library.applications.map(row=><tr key={row.id}><td>{row.effectiveFrom}</td><td>{row.templateName}</td><td>{statusLabels[row.status]}{row.error&&<p className="admin-help">{row.error}</p>}</td><td>{row.requestedByName}</td><td><button className="admin-button admin-button-secondary" disabled={busy} onClick={()=>run(async()=>{setHistory(await send("history",undefined,row.id));})}>변경 내역</button>{row.status==="APPLIED"&&<ApplicationDateCorrection divisionSlug={divisionSlug} application={row} onSaved={load}/>}{["PENDING","REVIEW"].includes(row.status)&&<button className="admin-button admin-button-secondary" disabled={busy} onClick={()=>run(async()=>{await send("cancel",undefined,row.id);await load();})}>예약 취소</button>}</td></tr>)}</tbody></table></div>}</section>
