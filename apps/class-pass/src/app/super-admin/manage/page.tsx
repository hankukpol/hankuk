...
export default function SuperAdminManagePage() {
  ...
  const [accountForm, setAccountForm] = useState({
    ...
    shared_user_id: '',
    pin: '',
    memberships: 'BRANCH_ADMIN:police',
  })
  ...
  return (
    <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-8 sm:px-6 lg:px-8">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="text-2xl font-extrabold text-slate-900">지점 / 운영자 관리</h2>
          <p className="mt-2 text-sm text-slate-500">
            기존 지점 로그인은 그대로 유지하면서, 포털 연동과 운영자 계정 관리를
            함께 정리하는 화면입니다.
          </p>
        </div>
        <button
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <section className="rounded-3xl bg-white p-6 shadow-sm">
          <h3 className="text-lg font-bold text-slate-900">지점 생성</h3>
        ...
        <section className="rounded-3xl bg-white p-6 shadow-sm">
          <h3 className="text-lg font-bold text-slate-900">운영자 계정 생성</h3>
          <p className="mt-2 text-xs leading-5 text-slate-500">
            멤버십 형식: <code>SUPER_ADMIN</code> 또는{' '}
      ...
      <section className="rounded-3xl bg-white p-6 shadow-sm">
        <h3 className="text-lg font-bold text-slate-900">지점 목록</h3>
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          {branches.map((branch) => (
            <div key={branch.id} className="rounded-2xl border border-slate-200 p-4">
              ...
              <p className="mt-3 text-sm leading-6 text-slate-600">{branch.description}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="rounded-3xl bg-white p-6 shadow-sm">
        <h3 className="text-lg font-bold text-slate-900">운영자 계정 목록</h3>
