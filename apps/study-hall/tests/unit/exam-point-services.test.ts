...
function fixture() {
  ...
  const state={
    ...
    examSessionsByDivision:{police:[{id:"session",divisionId:"police-id",examTypeId:"type",examDate:"2026-09-14",identityKey:"2026-09-14"}],fire:[]},
  ...
  const dependencies:Record<string,unknown>={
    ...
    "@/lib/exam-point-automation":calculator,"@/lib/management-policy":{...policy,kstDate:()=>"2026-09-14"},
...
test("repeated synchronization preserves point IDs; attendance correction revokes absence only",async()=>{
  ...
  assert.deepEqual(await service.syncExamPoints("police","2026-09-14","admin"),{grantedCount:2,revokedCount:0});
