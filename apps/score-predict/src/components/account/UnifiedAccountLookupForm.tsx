...
export default function UnifiedAccountLookupForm({ tenantType }: { tenantType: TenantType }) {
  ...
  if (tenantType === "fire") {
    return (
      <UserFormPageShell
        title="아이디 확인"
        description="소방 계정은 가입한 휴대전화 번호가 아이디입니다."
      >
        <div className="space-y-4">
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            <Button asChild size="lg"><Link href={withTenantPrefix("/login", tenantType)}>로그인</Link></Button>
            <Button asChild size="lg" variant="outline" className="border-slate-200 bg-white text-slate-700 hover:bg-slate-50 hover:text-slate-900"><Link href={withTenantPrefix("/forgot-password", tenantType)}>비밀번호 찾기</Link></Button>
          </div>
        </div>
      </UserFormPageShell>
    );
  ...
  return (
    <UserFormPageShell
      title="아이디 찾기"
      description="이름과 휴대전화가 일치하면 메일 인증 없이 아이디를 바로 안내합니다."
    >
      <div className="space-y-5">
        {usernames.length > 0 ? (
          <div className="space-y-4">
            ...
            <div className="grid gap-2 sm:grid-cols-2">
              <Button asChild size="lg"><Link href={withTenantPrefix("/login", tenantType)}>로그인</Link></Button>
              <Button asChild size="lg" variant="outline" className="border-slate-200 bg-white text-slate-700 hover:bg-slate-50 hover:text-slate-900">
          ...
          <form className="space-y-4" onSubmit={findUsername}>
            <div className="space-y-2">
              <Label htmlFor="lookupName" className="user-data-label">가입한 이름</Label>
              <Input id="lookupName" value={name} onChange={(event) => setName(event.target.value)} placeholder="홍길동" disabled={isBusy} required />
            </div>
            <div className="space-y-2">
              <Label htmlFor="lookupContactPhone" className="user-data-label">
            ...
            {legacyMode ? (
              <div className="space-y-2">
                <Label htmlFor="lookupPassword" className="user-data-label">기존 비밀번호</Label>
                <Input
                  id="lookupPassword"
                  type="password"
                  value={password}
