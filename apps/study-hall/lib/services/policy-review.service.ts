...
export async function getPolicyReviewSignals(divisionSlug: string) {
  ...
  if (isMockMode()) {
    ...
    const currentStudentIds = new Set((state.studentsByDivision[divisionSlug] ?? [])
      .filter((s) => s.status === "ACTIVE" || s.status === "ON_LEAVE").map((s) => s.id));
    points = (state.pointRecordsByDivision[divisionSlug] ?? [])
      .filter((r) => currentStudentIds.has(r.studentId)).map((r) => ({ ...r, date: r.date.slice(0, 10) }));
    phones = (state.phoneSubmissionsByDivision[divisionSlug] ?? []).filter((r) => currentStudentIds.has(r.studentId));
  } else {
    ...
    const [records, loans] = await Promise.all([
      prisma.pointRecord.findMany({ where: { student: { division: { slug: divisionSlug }, status: { in: ["ACTIVE", "ON_LEAVE"] } }, ruleId: { in: [policy.earlyExit.ruleId, policy.unauthorizedEntry.ruleId] }, date: { gte: new Date(`${earliest}T00:00:00Z`) } }, select: { studentId: true, ruleId: true, date: true } }),
      prisma.phoneSubmission.findMany({ where: { division: { slug: divisionSlug }, student: { division: { slug: divisionSlug }, status: { in: ["ACTIVE", "ON_LEAVE"] } }, date: { gte: new Date(`${earliest}T00:00:00Z`) } }, select: { studentId: true, rentalNote: true } }),
    ]);
  ...
  for (const studentId of ids) {
    ...
    const loans = phones.filter((r) => r.studentId === studentId)
      .flatMap((r) => Array.from((r.rentalNote ?? "").matchAll(/\[반출 ([^\]]+)\]/g)))
      .filter((match) => {
        const timestamp = new Date(match[1]);
        if (!Number.isFinite(timestamp.getTime())) return false;
        const date = kstDate(timestamp);
        return date >= policy.effectiveFrom && date >= since(policy.phone.repeatDays) && date <= today;
      }).length;
