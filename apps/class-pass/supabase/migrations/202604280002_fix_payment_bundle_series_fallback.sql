...
set series_label_snapshot = U&'\ACBD\CC44'
where series_group_snapshot = 'career'
  and coalesce(nullif(trim(series_label_snapshot), ''), U&'\ACF5\CC44') = U&'\ACF5\CC44';
...
begin
  ...
  for v_payment in select value from jsonb_array_elements(p_payments) loop
    ...
    if v_method not in ('card', 'cash', 'bank_transfer', 'point', 'free', 'other') then
    ...
    insert into class_pass.enrollment_payments (
      ...
      nullif(v_payment->>'bankAccountLast4', ''),
    ...
    insert into class_pass.payment_events (
      ...
      'payment_created',
      p_actor_staff_id,
      jsonb_build_object('payment', v_payment, 'payment_id', v_payment_id)
    );
