...
export function AdminRoomOpsPanel({
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`admin-room-ops-session-${sessionId}`)
      .on(
        "postgres_changes",
        {
          event: "*",
        ...
        () => {
          void refresh(selectedRoomId || undefined);
        },
      )
      .on(
        "postgres_changes",
  ...
  }, [loadWaiting, refresh, selectedRoomId, sessionId, supabase]);

  useEffect(() => {
    if (!supabase || !selectedRoomId) {
      return;
    }

    const channel = supabase
      .channel(`admin-room-ops-room-${selectedRoomId}`)
      .on(
        "postgres_changes",
        {
          event: "*",
  ...
  }, [loadDetail, refresh, selectedRoomId, supabase]);
  ...
  return (
    <>
      <SectionCard
        title="조 방 운영"
        description="관리자 페이지에서 조 방 상태, 조장, 추가 요청, 대기자 배정, 공지를 PC 중심으로 한 화면에서 관리합니다. 학생 `/room` 화면이 이 운영 변경과 공지를 그대로 소비합니다."
        action={
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Badge tone={supabase ? "success" : "neutral"}>
              {supabase ? "실시간 연결" : "실시간 미연결"}
            </Badge>
          </div>
        }
      >
          ...
          <div className="grid gap-4 xl:grid-cols-[minmax(0,0.78fr)_minmax(0,1.06fr)_minmax(280px,0.76fr)]">
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-[10px] border border-slate-200 bg-white p-4">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <p className="text-sm font-semibold text-slate-900">방 공지 및 채팅</p>
                    <p className="mt-1 text-sm text-slate-500">
                      관리자가 보낸 공지는 학생 방 화면 채팅에 시스템 메시지로 노출됩니다.
                    </p>
                  </div>
