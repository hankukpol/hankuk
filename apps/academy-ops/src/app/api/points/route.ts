import { AdminRole, PointType } from "@prisma/client";
import { NextResponse } from "next/server";
import { requireApiAdmin } from "@/lib/api-auth";
import { toAuditJson } from "@/lib/audit";
import { getPrisma } from "@/lib/prisma";

export const dynamic = "force-dynamic";

/**
 * POST /api/points
 * 관리자 포인트 지급 또는 차감 (통합 엔드포인트).
 * amount > 0: 지급, amount < 0: 차감
 * 차감 시 잔액 부족이면 오류.
 * Body: { examNumber: string, amount: number, reason: string, note?: string }
 * 권한: COUNSELOR+
 */
export async function POST(request: Request) {
  const auth = await requireApiAdmin(AdminRole.COUNSELOR);
  if (!auth.ok) return NextResponse.json({ error: auth.error }, { status: auth.status });

  try {
    const body = await request.json();
    const { examNumber, amount, reason, note } = body as {
      examNumber?: unknown;
      amount?: unknown;
      reason?: unknown;
      note?: unknown;
    };

    const cleanExamNumber = String(examNumber ?? "").trim();
    if (!cleanExamNumber) throw new Error("학번을 입력하세요.");

    const numAmount = Number(amount);
    if (!Number.isFinite(numAmount) || numAmount === 0)
      throw new Error("금액은 0이 아닌 숫자여야 합니다.");

    const cleanReason = String(reason ?? "").trim();
    if (!cleanReason) throw new Error("사유를 입력하세요.");

    // note는 PointLog에 별도 필드가 없으므로 reason에 합산
    const noteText = note ? String(note).trim() : "";
    const fullReason = noteText ? `${cleanReason} (${noteText})` : cleanReason;

    const prisma = getPrisma();

    const result = await prisma.$transaction(async (tx) => {
      const student = await tx.student.findUnique({
        where: { examNumber: cleanExamNumber },
        select: { name: true, examNumber: true },
      });
      if (!student) throw new Error(`학생을 찾을 수 없습니다: ${cleanExamNumber}`);

      // 차감 시 잔액 확인
      if (numAmount < 0) {
        const balanceResult = await tx.pointLog.aggregate({
          where: { examNumber: cleanExamNumber },
          _sum: { amount: true },
        });
        const currentBalance = balanceResult._sum.amount ?? 0;
        if (currentBalance + numAmount < 0) {
          throw new Error(
            `잔액 부족: 현재 잔액 ${currentBalance.toLocaleString()}P, 차감 요청 ${Math.abs(numAmount).toLocaleString()}P`,
          );
        }
      }

      const pointLog = await tx.pointLog.create({
        data: {
          examNumber: cleanExamNumber,
          type: PointType.MANUAL,
          amount: numAmount,
          reason: fullReason,
          grantedBy: auth.context.adminUser.name,
        },
      });

      await tx.auditLog.create({
        data: {
          adminId: auth.context.adminUser.id,
          action: numAmount > 0 ? "POINT_GRANT" : "POINT_DEDUCT",
          targetType: "PointLog",
          targetId: String(pointLog.id),
          before: toAuditJson(null),
          after: toAuditJson({
            examNumber: cleanExamNumber,
            amount: numAmount,
            reason: fullReason,
          }),
          ipAddress: request.headers.get("x-forwarded-for"),
        },
      });

      return { pointLog, student };
    });

    return NextResponse.json({ data: result.pointLog }, { status: 201 });
  } catch (error) {
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "처리 실패" },
      { status: 400 },
    );
  }
}
