    ...
    <section className="admin-section">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          {description ? <p className="admin-help mt-1">{description}</p> : null}
        </div>
        <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-1 text-xs font-semibold text-slate-700">
        ...
        {entries.map((entry, index) => (
          <article key={entry.id} className="rounded-lg border border-slate-200 bg-slate-50 p-4">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <label className="block">
                <span className="admin-label mb-2 block">결제일</span>
                <input
                  type="date"
              ...
              <label className="block">
                <span className="admin-label mb-2 block">결제 금액</span>
                <input
                  type="number"
                  value={entry.amount}
              ...
              <label className="block">
                <span className="admin-label mb-2 block">결제 수단</span>
                <PaymentMethodSelect
                  value={entry.method}
              ...
              <label className="block md:col-span-2">
                <span className="admin-label mb-2 block">결제 메모</span>
                <textarea
                  value={entry.notes}
