...
export function AppSwitchMenu({ role, divisionSlug = null }: AppSwitchMenuProps) {
  ...
  return (
    <details className="relative">
      <summary className="flex list-none cursor-pointer items-center gap-2 rounded-[12px] border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-sm transition hover:border-slate-300 hover:text-slate-950 [&::-webkit-details-marker]:hidden">
        앱 전환
      ...
      <div className="absolute right-0 z-50 mt-2 w-72 rounded-[16px] border border-slate-200 bg-white p-2 shadow-2xl shadow-slate-900/10">
          ...
          {quickTargets.length > 0 ? (
            quickTargets.map((target) => (
              <a
                ...
                className="flex items-center justify-between rounded-[12px] px-3 py-2.5 text-sm text-slate-700 transition hover:bg-slate-50 hover:text-slate-950"
              >
            ...
            <div className="rounded-[12px] bg-slate-50 px-3 py-3 text-sm text-slate-500">
        ...
        <div className="mt-2 border-t border-slate-100 pt-2">
          <a
            href={portalUrl}
            className="flex items-center justify-between rounded-[12px] px-3 py-2.5 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 hover:text-slate-950"
          >
