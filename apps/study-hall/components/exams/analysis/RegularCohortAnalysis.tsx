...
function SessionSelector({ base, examTypeId, initialDate }: { base: string; examTypeId: string; initialDate?:string }) {
  const id = useId();
...
export function RegularCohortAnalysis({ divisionSlug, examTypes, initialSelection = {} }: { divisionSlug: string; examTypes: ExamTypeItem[]; initialSelection?:Record<string,string> }) {
  const id = useId();
  ...
  return <div className="admin-flat-page"><div className="admin-filter-bar"><label className="admin-label" htmlFor={id}>시험 종류</label><select id={id} value={selected.id} onChange={(event) => setSelectedId(event.target.value)}>{examTypes.map((type) => <option key={type.id} value={type.id}>{type.name}</option>)}</select></div><SessionSelector initialDate={initialSelection.examDate} key={`${divisionSlug}-${selected.id}`} base={`/api/${encodeURIComponent(divisionSlug)}/exams/analysis`} examTypeId={selected.id} /></div>;
}
