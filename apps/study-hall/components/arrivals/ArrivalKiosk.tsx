...
export function ArrivalKiosk({ divisionSlug, divisionName }: { divisionSlug: string; divisionName: string }) {
  ...
  return <main className="admin-shell">
    <div className="admin-arrival-kiosk admin-flat-page">
      <header><p className="admin-label mb-2 break-keep">{divisionName}</p><h1 className="admin-page-title">등원 체크</h1><p className="admin-help mt-2">입력칸을 누른 뒤 기기의 숫자 키패드로 수험번호를 입력해 주세요.</p></header>
      ...
      {ready && config ? <form onSubmit={event => { event.preventDefault(); void submit(numberRef.current); }} className="space-y-4" aria-busy={saving}>
        <label className="block"><span className="admin-label mb-2 block">수험번호 {config.numberLength}자리</span>
          <input className="admin-arrival-number w-full" type="text" inputMode="numeric" pattern="[0-9]*" enterKeyHint="done" autoComplete="off" autoCorrect="off" spellCheck={false} value={number} maxLength={config.numberLength} disabled={saving || completion !== null} aria-describedby="arrival-kiosk-help" onChange={event => updateNumber(event.target.value)} />
        </label>
