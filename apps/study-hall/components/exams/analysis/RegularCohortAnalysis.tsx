...
function CohortSession({ base, examTypeId, examDate, view }: { base: string; examTypeId: string; examDate: string; view: CohortView }) {
  ...
  const numberById = new Map(analysis.ranking.map((row) => [row.studentId, row.studentNumber]));
  ...
  const ranking = analysis.ranking.filter(matches);
  const declines = analysis.declines.filter(matches);
  const partials = analysis.partials.filter(matches);
  return <div className="admin-flat-page">
    ...
    {view === "students" && <div className="admin-filter-bar"><StudentSearchField label="학생 검색" value={studentQuery} onChange={setStudentQuery} hint={searching ? `${ranking.length}명 표시 (전체 ${analysis.ranking.length}명)` : "이름을 누르면 개인 분석을 엽니다."} /></div>}
    {view === "students" && <section className="admin-section"><h2 className="admin-section-title">반 석차표</h2><p className="admin-help">직전 시험 대비 석차 상승은 ▲, 하락은 ▼로 표시합니다.</p>{ranking.length === 0 ? <p className="admin-empty-state">{searching ? "검색과 일치하는 학생이 없습니다." : "이 시험에 매칭된 반 학생이 없습니다."}</p> : <div className="admin-table-frame"><table><thead><tr><th scope="col">반 석차</th><th scope="col">이름</th><th scope="col">총점</th>{analysis.subjects.map((subject) => <th scope="col" key={subject.id}>{subject.name}</th>)}<th scope="col">외부 상위%</th><th scope="col">직전 시험 총점</th><th scope="col">직전 시험 석차</th><th scope="col">표시</th></tr></thead><tbody>{ranking.map((row) => <tr key={row.studentId}><td>{row.internalRank}등</td><td className="admin-table-name"><button type="button" className="admin-table-link" onClick={() => setStudent({ id: row.studentId, name: row.name })}>{row.name}</button></td><td>{row.totalScore}</td>{analysis.subjects.map((subject) => <td key={subject.id}>{row.subjectScores[subject.id] ?? (subject.alternateGroup && analysis.subjects.some((other) => other.alternateGroup === subject.alternateGroup && row.subjectScores[other.id] != null) ? "선택 안 함" : "미응시")}</td>)}<td>{number(row.externalTopPercent, "%")}</td><td>{row.delta ? delta(row.delta.total) : "—"}</td><td>{row.delta ? delta(row.delta.rank) : "—"}</td><td>{row.flags.some((flag) => flag.kind === "totalDrop" || flag.kind === "rankDrop") && <span className="admin-badge">하락</span>} {row.isPartial && <span className="admin-badge">일부 미응시</span>}</td></tr>)}</tbody></table></div>}</section>}
    {view === "students" && <section className="admin-section"><h2 className="admin-section-title">학습 확인 대상</h2>{!hasPreviousExam ? <p className="admin-empty-state">비교할 직전 시험이 없습니다.</p> : declines.length ? <ul className="admin-help">{declines.map((row) => <li key={row.studentId}>{row.name}: {row.flags.map((flag) => flag.detail).join(" · ")}</li>)}</ul> : <p className="admin-help">{searching ? "검색과 일치하는 학생이 없습니다." : "학습 확인 신호가 없습니다."}</p>}</section>}
    {view === "students" && <section className="admin-section"><h2 className="admin-section-title">일부 미응시 학생</h2>{partials.length ? <ul className="admin-help">{partials.map((row) => <li key={row.studentId}>{row.name}: {row.missingSubjects.join(", ")}</li>)}</ul> : <p className="admin-help">{searching ? "검색과 일치하는 학생이 없습니다." : "일부 미응시 학생이 없습니다."}</p>}</section>}
  </div>;
}
