...
export function buildPolicyAttendanceCandidates(policy: ManagementPolicy, periods: PolicyPeriod[], records: PolicyAttendance[], rules: { id: string; points: number; isActive: boolean }[], date: string, now = new Date()): AttendancePenaltyCandidate[] {
  ...
  const add = (studentId: string, ruleId: string | null, scope: string, label: string) => {
    const rule = ruleId ? activeRules.get(ruleId) : null;
    if (rule && rule.points < 0) candidates.push({ studentId, ruleId: rule.id, points: rule.points, notes: `[자동][출결벌점][${date}][${scope}][${scope === "DAY" ? "ABSENT" : "TARDY"}] ${label}` });
  };
  for (const studentId of Array.from(studentIds)) {
    ...
    if (dayEnded && controlled.every((p) => byPeriod.get(p.id)?.status === "ABSENT")) {
      add(studentId, policy.fullDayAbsenceRuleId, "DAY", "관리일 전체 무단결석 (교시별 중복 없음)");
      continue;
    }
    for (const p of controlled) {
      ...
      if (status === "TARDY") add(studentId, policy.tardyRuleId, p.id, `${p.name} 시작 후 도착`);
      if (status === "ABSENT" && policy.partialAbsenceRuleId) add(studentId, policy.partialAbsenceRuleId, p.id, `${p.name} 결석`);
    }
  }
  return candidates;
}
