...
function parse(files: ExamImportFiles, selection: ExamImportSelection) {
  try {
...
export async function previewExamImport(
  ...
  return assemble(source, parse(files, selection), selection).preview;
}
...
export async function confirmExamImport(
  ...
  const result = await prisma.$transaction(
    async (tx) => {
      ...
      const assembly = assemble(source, parse(files, selection), selection);
