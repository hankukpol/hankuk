...
function ReviewMethod() {
  return <section className="admin-section"><h3 className="admin-section-title">복습 실행 방법</h3>
    ...
    <p className="admin-help">정답률은 이 시험 응시 집단 기준입니다.</p>
    <p className="admin-help admin-print-only">복습 기록: 확인한 원인 __________ · 교재/페이지 __________ · 재풀이 날짜 __________ · 재풀이 결과 __________ (출력 후 작성)</p>
  </section>;
}
...
export function RegularLearningSummary({ report }: { report: RegularStudentReport }) {
  ...
  return <section className="admin-section" data-learning-summary id="personal-diagnosis">
    ...
    <p className="admin-help">쉬운 문항 오답 수와 잃은 점수 순으로 복습할 과목을 안내합니다.</p>
    ...
    <p className="admin-help">쉬움 기준은 설정에서 정한 외부 정답률입니다.</p>
...
export function MorningLearningSummary({ report }: { report: MorningStudentReport }) {
  ...
  return <section className="admin-section" data-learning-summary id="personal-diagnosis">
    ...
    <p className="admin-help">입력된 단원과 문항 정오를 근거로 복습 과제를 제시합니다.</p>
    ...
    {plan.map(subject => <section className="admin-section" key={subject.subjectId}>
      ...
      {subject.withheld && <p className="admin-notice">응시율 또는 비교 기록이 부족해 추세 판정을 보류합니다. 아래는 실제 응시한 문항의 복습 목록입니다.</p>}
