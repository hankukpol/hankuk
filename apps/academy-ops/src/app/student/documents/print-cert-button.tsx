...
export function PrintCertButton({
  label = "인쇄하기",
  certClass,
}: PrintCertButtonProps) {
  function handlePrint() {
    // Hide all cert sections, then reveal only the target one
    const allCerts = document.querySelectorAll<HTMLElement>(".printable-cert");
    const target = document.querySelector<HTMLElement>(`.${certClass}`);
